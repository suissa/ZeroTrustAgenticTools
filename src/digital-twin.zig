...
pub fn main() !void {
    try common.run(.{
        .name = "ztat-digital-twin",
        .domain = "digital twin",
        .purpose = "preference modeling, behavior simulation, decision rehearsal, memory review",
        .capabilities = &.{ "preference modeling, behavior simulation, decision rehearsal, memory review", "validates all input as untrusted", "returns plan-only JSON by default", "requires scoped capability tokens for side effects" },
