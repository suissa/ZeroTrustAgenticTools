...
pub fn main() !void {
    try common.run(.{
        .name = "ztat-personal-shopper",
        .domain = "personal shopper",
        .purpose = "requirements capture, comparison matrix, budget fit, purchase risk notes",
        .capabilities = &.{ "requirements capture, comparison matrix, budget fit, purchase risk notes", "validates all input as untrusted", "returns plan-only JSON by default", "requires scoped capability tokens for side effects" },
