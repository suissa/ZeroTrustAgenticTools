...
pub fn main() !void {
    try common.run(.{
        ...
        .purpose = "budgeting, cash-flow, reconciliation hints, portfolio risk triage",
        .capabilities = &.{ "budgeting, cash-flow, reconciliation hints, portfolio risk triage", "validates all input as untrusted", "returns plan-only JSON by default", "requires scoped capability tokens for side effects" },
