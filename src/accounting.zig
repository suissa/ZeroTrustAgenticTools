...
pub fn main() !void {
    try common.run(.{
        .name = "ztat-accounting",
        .domain = "accounting operations",
        .purpose = "ledger classification, invoice checks, close checklist, tax evidence pack",
        .capabilities = &.{ "ledger classification, invoice checks, close checklist, tax evidence pack", "validates all input as untrusted", "returns plan-only JSON by default", "requires scoped capability tokens for side effects" },
